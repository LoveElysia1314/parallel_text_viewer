...
// 页面标题（由后端渲染时填入，含完整层级：章节名 - 卷名 - 作品名）
...
requestAnimationFrame(() => {
  ...
  if (lastActivePair !== null) {
    const p = content.querySelector(
      `.pair-wrapper[data-pair='${lastActivePair}']`,
    );
    if (p) {
...
if (toggleOrientation) {
  toggleOrientation.addEventListener("click", () => {
    ...
    requestAnimationFrame(() => {
      const targetWrapper = content.querySelector(
        `.pair-wrapper[data-pair='${currentIdx}']`,
      );
      if (targetWrapper) {
...
function scheduleRecomputeHeights() {
  ...
  _recomputeTimer = setTimeout(() => {
    Object.keys(pairHeights).forEach((k) => delete pairHeights[k]);
    const visiblePairs = Array.from(content.querySelectorAll(".pair-wrapper"))
      .filter((p) => p.style.display !== "none")
      .map((p) => Number(p.dataset.pair));
...
function render() {
  ...
  pairs.forEach((p, i) => {
    ...
    mainBlock.querySelector(".text").innerHTML =
      pairs[i][localPrimary] + "\n\n";
    sideBlock.querySelector(".text").innerHTML =
      pairs[i][1 - localPrimary] + "\n\n";
  ...
  if (typeof IntersectionObserver !== "undefined") {
    pairObserver = new IntersectionObserver(
      (entries) => {
        const toMeasure = [];
        entries.forEach((en) => {
          if (en.isIntersecting) toMeasure.push(Number(en.target.dataset.pair));
        });
        if (toMeasure.length) applyEqualHeightsForPairs(toMeasure);
      },
      { root: null, rootMargin: "500px 0px" },
    );
    Array.from(content.querySelectorAll("[data-pair]")).forEach((n) =>
      pairObserver.observe(n),
    );
  }
  const visiblePairs = Array.from(content.querySelectorAll("[data-pair]"))
    .filter((p) => p.style.display !== "none")
    .map((p) => Number(p.dataset.pair));
  if (visiblePairs.length) applyEqualHeightsForPairs(visiblePairs);
}
...
function updateProgress() {
  const visible = Array.from(document.querySelectorAll("[data-pair]")).filter(
...
function getPairPrimaryIdx(i) {
  if (
    tempOverrideIdx === i &&
    typeof tempOverridePrimary !== "undefined" &&
    tempOverridePrimary !== null
  )
    return tempOverridePrimary;
...
function updatePageTitle() {
  ...
  const langSuffix = displayDoc === "b" ? " [EN]" : "";
  // h2 显示当前主要语言的标题，标签页显示完整层级
  const displayTitle = titles[displayDoc] || "__TITLE__";
  const h2 = document.querySelector("h2");
  if (h2) h2.textContent = displayTitle;
  // 标签页使用完整页面标题（作品名 - 卷名 - 章节名）+ 语言指示
  document.title = pageTitle + langSuffix;
}
...
function renderPair(i) {
  ...
  const sideBlock = wrapper.querySelector(".block-side");
  const localPrimary = getPairPrimaryIdx(i);
  mainBlock.querySelector(".text").textContent =
    pairs[i][localPrimary] + "\n\n";
  sideBlock.querySelector(".text").textContent =
    pairs[i][1 - localPrimary] + "\n\n";
  wrapper.classList.toggle("horizontal", orientation === "horizontal");
  const isExpanded = !!expandedPairs[i];
  wrapper.classList.toggle("expanded", isExpanded);
...
function indexToPercent(idx) {
  ...
  const visiblePairs = Array.from(
    content.querySelectorAll(".pair-wrapper"),
  ).filter((p) => p.style.display !== "none");
  if (visiblePairs.length <= 1) return 0;
  // 找到当前idx在可见pairs中的位置
  const visibleIndices = visiblePairs
    .map((p) => Number(p.dataset.pair))
    .sort((a, b) => a - b);
...
function goToPercent(pct) {
  ...
  if (!target || target.style.display === "none") {
    ...
    const closestVisible = visiblePairs.reduce((closest, current) => {
      ...
      return Math.abs(currentIdx - idx) < Math.abs(closestIdx - idx)
        ? current
        : closest;
    });
...
document.addEventListener("keydown", (e) => {
  if (e.key === "p" || e.key === "P") {
    if (togglePrimaryDoc) togglePrimaryDoc.click();
  }
  if (e.key === "s" || e.key === "S") {
    if (toggleSync) toggleSync.click();
  }
  if (e.key === "o" || e.key === "O") {
    if (toggleOrientation) toggleOrientation.click();
  }
  if (e.key === "ArrowUp") {
    e.preventDefault();
...
if (positionSlider) {
  positionSlider.addEventListener("input", (e) => {
    ...
    setTimeout(() => {
      isUpdatingPositionFromScroll = false;
    }, 150);
  });
}
