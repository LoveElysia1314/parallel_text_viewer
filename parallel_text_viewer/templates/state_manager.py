...
function syncStateToUI() {
  ...
  if (widthSlider) {
    ...
    widthInput.value = state.containerWidth;
  ...
  if (positionSlider) {
    ...
    posInput.value = parseFloat(state.panelScrollPercent).toFixed(2);
  }
  ...
  if (toggleOrientation) {
    toggleOrientation.textContent = state.orientation === 'vertical' ? 'Layout: Vertical' : 'Layout: Horizontal';
  ...
  if (clickActionSel) {
    clickActionSel.value = state.clickAction;
  }
...
// 监听状态变化并应用到 UI
stateManager.on('load', (data) => {
  console.log('State loaded:', data);
  syncStateToUI();
});

stateManager.on('change', (change) => {
  console.log(`State changed: ${change.key}`, change);
});

stateManager.on('error', (err) => {
  console.error('State manager error:', err);
});
"""
...
def get_state_manager_code(doc_id: str) -> str:
    ...
    Returns:
        替换后的 JavaScript 代码
    """
    return STATE_MANAGER_JS.replace("__DOC_ID__", doc_id)
