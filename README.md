...
│   └── dispatcher.py        # 命令分发
├── core/                    # 核心功能
│   ├── text_processor.py    # 文本解析与行对齐
│   ├── config_v2.py         # 配置管理
│   ├── path_resolver.py     # 路径解析
│   └── validator.py         # 配置验证
├── generators/              # HTML 生成器 (v2)
│   ├── factory.py           # 工厂模式
│   ├── single_file.py       # 单文件生成器
│   ├── book_index.py        # 书籍索引生成器
│   └── navigation.py        # 导航栏
├── templates/               # HTML 模板
│   ├── chapter_template.py  # 章节页面
│   ├── index_template.py    # 索引页面
│   ├── common_template.py   # 通用样式/脚本
│   └── renderer.py          # 模板引擎
└── utils/                   # 工具函数
    ├── file_utils.py
    ├── image_utils.py
...
A: 修改 `parallel_text_viewer/templates/` 中的模板文件，重新生成 HTML 即可应用新样式。
...
- **语言**：Python 3.7+
- **依赖**：仅使用标准库（pathlib、json、re、logging）
